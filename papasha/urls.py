...
from papasha import app, bcrypt,session
...
def verify_password(useremail, userpass):
    user = session.query(User).filter(User.useremail == useremail).first()
    if user is None:
        return False
    else:
        return bcrypt.check_password_hash(user.userpass, userpass)
...
def user_add():
    ...
    try:
        data['userpass'] = bcrypt.generate_password_hash(
            data['userpass']).decode('utf-8')
...
def update_user(pk):
    ...
    s = "userpass"
    if s in data:
        data["userpass"] = bcrypt.generate_password_hash(data["userpass"]).decode('utf-8')
    try:
        user = g404(User, pk)
    except Exception:
